...
O modelo TBATS foi o que apresentou o menor erro absoluto médio no conjunto de dados de teste (`r round(f_tbats[2,3],3)`). O modelo BATS apresentou o segundo menor valor, seguido pelo STLF e ETS, entretanto este último foi avaliado por um modelo sem tendência, ao contrário da modelagem da série original. 
.
```{r, include = F}
ets(serie)
```
...
A série ID 1686 do banco de dados M3, é uma série que apresenta tendência negativa e sazonalidade. Ela é tornada estacionária com uma diferenciação simples. A modelagem da série por modelos ARIMA e ETS apresenta resultados preditivos semelhantes, com erros absolutos médios próximos para previsões em cinco horizontes. A análise preditiva, utilizando um segundo conjunto de dados de teste, mostrou que os benchmarks TBATS, BATS, STLF e ETS apresentaram os menores erros absolutos médios
