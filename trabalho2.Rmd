...
fit_ets_boxcox <- ets(BoxCox(serie,lambda=lambda_boxcox), model = "MAM", damped = T)
```
...
```
Percebe-se que o Erro Absoluto Médio apresentado pelo modelo ETS com transformação Box-Cox é muito superior aos demais. Para melhor compreensão do comportamento dos erros dos demais modelos, o gráfico abaixo ilustra compara os erros de cada modelo para os cinco horizonte de previsão com os erros de cada modelo, excluindo o modelo ETS com BoxCox:
...
legend(x=1,y=1200, legend=c("ARIMA", "ARIMA COM BOXCOX", "ETS", "ETS com BOXCOX"), col=1:4, lwd=c(2,2), cex=1)
```
...
f_ets_boxcox = fit_ets_boxcox %>% forecast(h=h) %>% accuracy(dados_teste)
...
modelos = c("arima","arima com boxcox","ets","ets com boxcox","auto arima","ses","holt","auto ets","stlf","bats","tbats")
