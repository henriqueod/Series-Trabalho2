...
```
Os resultados acima mostram que a independência dos resíduos pode ser rejeitada ao nível de significância de 5% em ambos os modelos. 
  ...
  for(comp_tend in componentes_tendencia){
    for(comp_saz in componentes_sazonais){
      for(damped in c(T,F)){
        ...
        print("damped=")
        print(damped)
        modelo_ets = model_ets(BoxCox(serie, 
                                      lambda = BoxCox.lambda(serie)),
                               model=modelo, damped = damped)
        print(modelo_ets['aicc'])
      }
    }
  }
...
fit_ets_boxcox <- ets(BoxCox(serie,lambda = BoxCox.lambda(serie)), model = "MMM", damped = FALSE)
```
...
par(mfrow=c(2,2)); 
plot(residuo_ets, xlab = "Tempo", ylab = "");
acf(residuo_ets, main="");
pacf(residuo_ets, main="");
qqnorm(residuo_ets, main="");qqline(residuo_ets)
```
...
|---|---|:---:|
| KPSS | `r round(kpss_ets$statistic,3)` | `r round(kpss_ets$p.value,3)` |
| Box-Ljung | `r round(box_ets$statistic,3)` | `r round(box_ets$p.value,3)` |
| Shapiro-Wilk | `r round(shap_ets$statistic,3)` | `r round(shap_ets$p.value,3)` |


Agora analisando o modelo ETS(M,M,M), com a transformação boxcox, temos os seguintes resultados:



```{r, fig.align="center", fig.cap="Análise de Resíduos do ETS com boxcox", echo = FALSE}
residuo_ets_boxcox <- fit_ets_boxcox$residuals
par(mfrow=c(2,2)); 
plot(residuo_ets_boxcox, xlab = "Tempo", ylab = "");
acf(residuo_ets_boxcox, main="");
pacf(residuo_ets_boxcox, main="");
qqnorm(residuo_ets_boxcox, main="");qqline(residuo_ets_boxcox)
...
| Teste | Valor do teste | P-valor |
|---|---|:---:|
| KPSS | `r round(kpss_ets_boxcox$statistic,3)` | `r round(kpss_ets_boxcox$p.value,3)` |
| Box-Ljung | `r round(box_ets_boxcox$statistic,3)` | `r round(box_ets_boxcox$p.value,3)` |
| Shapiro-Wilk | `r round(shap_ets_boxcox$statistic,3)` | `r round(shap_ets_boxcox$p.value,3)` |
...
```{r, , fig.align="center", fig.cap="Previsão ETS com transformação boxcox", echo = FALSE}
fit_ets %>% forecast(h=12) %>% plot(main="Previsão ETS 'MMM' com transformação boxcox")
```
...
fit_auto_arima = auto.arima(serie) %>% forecast() %>% accuracy()
fit_ses = ses(serie, h=h)  %>% forecast() %>% accuracy()
fit_holt = holt(serie, h=h) %>% forecast() %>% accuracy()
fit_ets = ets(serie) %>% forecast() %>% accuracy()
fit_stlf = stlf(serie, h=h) %>% forecast() %>% accuracy()
fit_bats = bats(serie) %>% forecast() %>% accuracy()
fit_tbats = tbats(serie) %>% forecast() %>% accuracy()

acuracia <- rbind(fit_auto_arima[3],
                  fit_ses[3],
                  fit_holt[3],
                  fit_ets[3],
                  fit_stlf[3],
...
modelos = c("auto arima","ses","holt","ets","stlf","bats","tbats")

resultados = cbind(modelos, round(acuracia,3))

kable(resultados,col.names = c("Modelo", "Acuracia"),align = "c")
```
