...
fit_ets_boxcox <- ets(BoxCox(serie,lambda = "auto"), model = "MAM", damped = TRUE)
```
...
f_arima_boxcox <- function(y, h){
  forecast(fit_arima_boxcox, h)
}
f_ets <- function(y, h){
  forecast(fit_ets, h)
}
f_ets_boxcox <- function(y, h){
  forecast(fit_ets_boxcox, h)
}
...
```
Percebe-se que o Erro Absoluto Médio apresentado pelo modelo ETS com transformação Box-Cox é muito superior aos demais. Para melhor compreensão do comportamento dos erros dos demais modelos, o gráfico abaixo ilustra os erros de cada modelo para os cinco horizonte de previsão:
...
plot.ts(tab[,1:3], plot.type='s', col=1:5, lwd=c(2,2), xlab="h",ylab="MAE")
legend(x=1,y=1200, legend=c("ARIMA", "ARIMA COM BOXCOX", "ETS"), col=1:5, lwd=c(2,2))
```
